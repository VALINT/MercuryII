...
#define SELECT()	_spi_enableSS_m	    /* CS = L */
#define DESELECT()	_spi_disableSS_m	/* CS = H */
#define MMC_SEL	    _spi_statSS_m   	/* CS status (true:CS == L) */
#define FORWARD(d)	         		            /* Data forwarding function (Console out in this example) */

#define init_spi()  _spi_init()     	        /* Initialize SPI port (usi.S) */
#define dly_100us() _delay(100)	                /* Delay 100 microseconds (usi.S) */
#define xmit_spi(d) _spi_writeByte_m(d)         /* Send a byte to the MMC (usi.S) */
#define rcv_spi()	_spi_readByte()           /* Send a 0xFF to the MMC and get the received byte (usi.S) */
...
void _delay(unsigned int nCount)
{
	unsigned int i, j;
